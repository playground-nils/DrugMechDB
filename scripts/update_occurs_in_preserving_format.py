...
Changes the predicate to "phenotype of" (keeps same direction).
"""
...
def update_file_preserving_format(filepath: str, links_to_update: Set[tuple]) -> int:
    ...
    Changes predicate from "occurs in" to "phenotype of".
            ...
            for j in range(i + 1, min(i + 5, len(lines))):
                ...
                if source_id and target_id:
                    if (source_id, target_id) in links_to_update:
                        ...
                        changes_count += 1
                        print(f"Updated line {i + 1}: {source_id} -> {target_id}, changed to 'phenotype of'")
                    break
